...
def send_sms_to_nearby_suppliers(ussd_request_obj):
    ...
    if ussd_request_obj.order_type == USSDRequest.PRODUCT_ORDER:
        ...
        suppliers = get_suppliers_nearby(
            ussd_request_obj.user.farmer_profile.geom, 
            supplier_queryset=suppliers
        )
        if suppliers:
        ...
        suppliers = get_suppliers_nearby(
            ussd_request_obj.user.farmer_profile.geom, supplier_queryset=suppliers
        )
        if suppliers:
            for index, supplier in enumerate(suppliers):
                message += "\n%s.%s %s" % (
